...
//  ✅ Auto-clicker tick smoothed to 250ms micro-ticks
//  ✅ Prestige threshold raised: 5,000 → 10,000
//  ✅ Box prices rebalanced (Snack 10→8, Gourmet 50→40)
//  ✅ Achievements persist across prestige (no reset)
//  ✅ Duplicate achievement unlock bug fixed
//  ✅ CPS badge updates instantly after buying upgrade
//  ✅ Sell Dupes floater position fixed
//  ✅ Coin label shows multiplier when prestiged
//  ✅ Settings info panel added
// ════════════════════════════════════════════════════════
    ...
    switch (type) {
      ...
      case "legendary":
        [0, 0.1, 0.2].forEach((d, i) => {
          ...
          o.type = "sine";
          o.frequency.setValueAtTime(600 + i * 200, now + d);
          o.frequency.exponentialRampToValueAtTime(1400 + i * 100, now + d + 0.25);
      ...
      case "achievement":
        [0, 0.12, 0.24].forEach((d, i) => {
          ...
          o.connect(g); g.connect(ctx.destination);
          o.type = "triangle";
          o.frequency.setValueAtTime(700 + i * 150, now + d);
      ...
      case "prestige":
        [0, 0.1, 0.2, 0.3, 0.4].forEach((d, i) => {
          ...
          o.connect(g); g.connect(ctx.destination);
          o.type = "sine";
          o.frequency.setValueAtTime(400 + i * 180, now + d);
          o.frequency.exponentialRampToValueAtTime(1600 + i * 80, now + d + 0.35);
          g.gain.setValueAtTime(0.2, now + d);
...
const BOXES = [
  ...
  { name: "Ultra Box",   icon: "💜", price: 600,  btnClass: "b3", desc: "Guaranteed Epic or higher. No common drops.",   odds: "Epic 45% · Legendary 35% · Mythic 20%",        weights: { common:0,  uncommon:0,  rare:0,  epic:45,  legendary:35,  mythic:20  }, cardClass: "ultra-card"  },
...
// ── RECIPES ────────────────────────────────────────────
const RECIPES = [
  { id: "breakfast_royale", name: "Breakfast Royale",  icon: "🍳", desc: "The ultimate breakfast — eggs, bread and pancakes stacked to perfection.", ingredients: [{ name:"Boiled Egg",qty:2},{name:"Plain Bread",qty:1},{name:"Pancakes",qty:1}], sellValue: 30   },
  ...
  { id: "dragon_sushi",     name: "Dragon Sushi Roll", icon: "🐉", desc: "Dragon Ramen broth poured over a premium sushi platter.",                 ingredients: [{ name:"Dragon Ramen",qty:1},{name:"Sushi Platter",qty:2}],                   sellValue: 450  },
  { id: "cosmic_platter",   name: "Cosmic Platter",    icon: "🌌", desc: "Celestial Ramen meets Star Cake — a meal that transcends dimensions.",     ingredients: [{ name:"Celestial Ramen",qty:1},{name:"Star Cake",qty:1},{name:"Ambrosia Bowl",qty:1}], sellValue: 5000 },
];
RECIPES.forEach(r => {
...
// ── PRESTIGE ──────────────────────────────────────────
const PRESTIGE_THRESHOLD = 10000; // raised from 5,000
const PRESTIGE_MULTIPLIERS = [1.0, 1.5, 2.0, 2.75, 3.5, 5.0, 7.0, 10.0];

// ── ACHIEVEMENTS ──────────────────────────────────────
const ACHIEVEMENTS = [
  { id: "first_click",  icon: "👆", name: "First Touch",     desc: "Click the coin once.",           cat: "clicks",     type: "clicks",      target: 1,           reward: 5    },
...
// ── SMOOTH AUTO-CLICKER TICK (250ms) ──────────────────
setInterval(() => {
  const cps = getTotalCps();
  if (cps > 0) {
...
function buyUpgrade(idx) {
  ...
  updateMoney();
  updateCpsBadge(); // fix: instant update
...
function sellAllDupes() {
  ...
  if (total > 0) {
    ...
    playSound("sell");
    // fix: correct floater position using getBoundingClientRect
...
function doPrestige() {
  ...
  playSound("prestige");
  // fix: achievements NOT reset on prestige
  checkAchievements();
...
// fix: dark mode toggle now correctly targets body
...
function checkAchievements() {
  ...
  ACHIEVEMENTS.forEach(ach => {
    if (achievementsDone.has(ach.id)) return; // guard prevents re-firing
    let done = false;
    switch (ach.type) {
    ...
    if (done) {
      achievementsDone.add(ach.id); // added before reward — prevents double-fire
      money += ach.reward;
      updateMoney();
